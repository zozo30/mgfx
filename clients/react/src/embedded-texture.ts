...
export function embeddedTexture(dataUrl: string): EmbeddedTexture {
  ...
  const match = dataUrl.match(/^data:image\/(png|jpeg|svg\+xml);base64,([A-Za-z0-9+/=\s]+)$/i);
  if (!match) throw new Error("SVG image requires an embedded base64 PNG, JPEG, or SVG data URL");
