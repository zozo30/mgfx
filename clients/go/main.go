...
func main() {
	...
	app.Draw = func(canvas *mgfx.Canvas) {
		...
		labelBackground := mgfx.Rect{
			X: banner.X + (banner.Width-labelWidth)/2 - 18 + textOffset,
			Y: canvas.Size.Height/2 - 27, Width: labelWidth + 36, Height: 50,
		}
		...
		canvas.Text("Hello from Go over MGFX", mgfx.TextStyle{
			X: banner.X + banner.Width/2 + textOffset,
